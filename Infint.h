...
using VectorType = std::vector<ELEM_TYPE>;
...
public:
    ...
    const InfInt& operator=(InfInt&& l) noexcept;
...
private:
    ...
    static void multiplyByDigit(ELEM_TYPE factor, VectorType& val);
    ...
    VectorType val; // number with base FACTOR
...
inline InfInt::InfInt(InfInt&& l) noexcept : pos(l.pos), val(std::move(l.val))
{
}
...
inline const InfInt& InfInt::operator=(InfInt&& l) noexcept
{
    //PROFINY_SCOPE
    pos = l.pos;
    val = std::move(l.val);
...
inline void InfInt::multiplyByDigit(ELEM_TYPE factor, VectorType& val)
{
    //PROFINY_SCOPE
    ELEM_TYPE carry = 0;
    for (size_t i = 0; i < val.size(); ++i)
